import sys
from .exceptions import BrainPySyntaxError, BrainPyRuntimeError
...
class BrainPy:
    ...
    def __init__(self, memory_size=30000):
        """
        Initialize the BrainPy interpreter.
    ...
    def _validate_code(self, code):
        ...
        Raises:
            BrainPySyntaxError: If brackets are unbalanced
        """
        stack = []
        for i, char in enumerate(code):
            if char == '[':
                ...
                if not stack:
                    raise BrainPySyntaxError(f"Unmatched ']' at position {i}")
                stack.pop()
        
        if stack:
            raise BrainPySyntaxError(f"Unmatched '[' at position {stack[0]}")
    ...
    def compile(self, brainfuck_code):
        ...
        Raises:
            BrainPySyntaxError: If the Brainfuck code has syntax errors
        """
    ...
    def execute(self, brainfuck_code, input_data=""):
        ...
        Raises:
            BrainPySyntaxError: If the Brainfuck code has syntax errors
            BrainPyRuntimeError: If there's a runtime error during execution
        """
        try:
            ...
            raise BrainPyRuntimeError(f"Runtime error: {str(e)}")
